...
##./isaaclab.sh -p source/standalone/sky/01_data_collect.py --task Isaac-Lift-Cube-ur5e-v2-IK-Rel-v0 --num_envs 1 --num_demos 10 --device keyboard
## if run ./isaaclab.sh -p source/standalone/sky/01_data_collect.py
## default is Isaac-Lift-Cube-ur5e-v2-IK-Rel-v0, num env 1, demo 10
## data store at /home/simtech/IsaacLab/logs/robomimic/Isaac-Lift-Cube-ur5e-v2-IK-Rel-v0
...
parser.add_argument("--task", type=str, default="Isaac-Lift-Cube-UR5e-IK-Rel-v0", help="Name of the task.")
parser.add_argument("--control", type=str, default="keyboard", help="Device for interacting with environment")

parser.add_argument("--num_demos", type=int, default=10, help="Number of episodes to store in the dataset.")
...
# Use the renamed argument for teleoperation
if args_cli.control.lower() == "keyboard":
    teleop_interface = Se3Keyboard(pos_sensitivity=0.04, rot_sensitivity=0.08)
elif args_cli.control.lower() == "spacemouse":
    teleop_interface = Se3SpaceMouse(pos_sensitivity=0.05, rot_sensitivity=0.005)
else:
    raise ValueError(f"Invalid control device interface '{args_cli.control}'. Supported: 'keyboard', 'spacemouse'.")
...
def main():
    ...
    assert (
        args_cli.task == "Isaac-Lift-Cube-UR5e-IK-Rel-v0"
    )
    # parse configuration
